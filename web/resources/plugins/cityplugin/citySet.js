$style($ctx+'/resources/plugins/cityplugin/city-picker.css');
function SelCity(obj,e) {
    $(obj).removeAttr("data-harea");
    var ths = obj;
    var arrStr = $(obj).val().split('-');

    var dal = '<div class="_citys"><span id="cSure" title="确定" class="mr50 w40 h-20">确定</span><span title="关闭" id="cColse" >×</span><ul id="_citysheng" class="_citys0"><li class="citySel">省份</li><li>城市</li><li>区县</li></ul><div id="_citys0" class="_citys1"></div><div style="display:none" id="_citys1" class="_citys1"></div><div style="display:none" id="_citys2" class="_citys1"></div></div>';
    ...
    $("#cSure").click(function () {
        ...
        if (len == 3){
            $(obj).attr("data-harea",$("#harea").attr('data-id'));
        }else if(len ==2){
            $(obj).attr("data-harea",$("#hproper").attr('data-id').substring(0,4));
        }else{
            $(obj).attr("data-harea",$("#hcity").attr('data-id').substring(0,2));
        }
    });
    var tb_province = [];
    var b = province;
    ...
    if(arrStr.length > 0){
        $("#_citys0 a").each(function () {
            if($(this).attr('data-name') == arrStr[0]){
                var g = getCity($(this));
                $("#_citys1 a").remove();
                $("#_citys1").append(g);
                $("._citys1").hide();
                ...
                $("#_citysheng li:eq(0)").addClass("citySel");
    ...
    if(arrStr.length > 1){
        $("#_citys1 a").each(function () {
            if($(this).attr('data-name') == arrStr[1]){
    ...
    if(arrStr.length > 2){
        $("#_citys2 a").each(function () {
            if($(this).attr('data-name') == arrStr[2]){
    ...
    $("._citys").on('click','#_citys0 a',function () {
        var g = getCity($(this));
        $("#_citys1 a").remove();
        $("#_citys1").append(g);
        $("._citys1").hide();
    ...
    $("._citys").on('click','#_citys1 a',function () {
        $("#_citys1 a,#_citys2 a").removeClass("AreaS");
        $(this).addClass("AreaS");
        var lev =  $(this).data("name");
        if (document.getElementById("hproper") == null) {
    ...
    $("._citys").on('click','#_citys2 a',function () {
        $("#_citys2 a").removeClass("AreaS");
        $(this).addClass("AreaS");
        var lev = $(this).data("name");
        if (document.getElementById("harea") == null) {
...
function getCityName(str) {
    var cityId0,cityId1,cityId2 = '';
    var cityName0 = '',cityName1='',cityName2 = '';
    var city = null;
    cityId0 = str.substring(0,2);
    cityId1 = str.substring(2,4);
    cityId2 = str.substring(4,6);
    if (cityId0){
        for(var i=0; i<province.length; i++){
            if(province[i].id.substring(0,2) == cityId0){
                city = province[i].city;
                cityName0 = province[i].name;
                break;
            }
        }
        if (cityId1){
            for(var i=0; i<city.length; i++){
                if(city[i].id.substring(2,4) == cityId1){
                    cityName1 = '-'+city[i].name;
                    break;
                }
            }
        }
        if(cityId2){
            for (var i=0; i<area.length; i++){
                if(area[i].id.substring(4,6) == cityId2 && area[i].id.substring(2,4)==cityId1 && area[i].id.substring(0,2)==cityId0){
                    cityName2 = '-'+area[i].name;
                    break;
                }
            }
        }
    }
    return cityName0+cityName1+cityName2;
}
...
var Iput = {
    ...
    hide: function (e) {//点击任何处关闭层
        ...
        if (Iput.confg.event == undefined) {//输入时用,般在没传入Iput.confg.event请况下使用
            ...
            if (a != srcElement) { $("#cSure").click();Iput.colse(); }
            if (b != null) {
